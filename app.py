# VERSAO_FINAL_SOCIAL_PRODUCAO_V2_SECRETS
...
# --- MOTOR DE EXTRAÇÃO DE DADOS SOCIAIS (API REAL + SECRETS) ---
def extrair_dados_sociais(sistema, uf, municipio, ano, mes):
    ...
    if sistema == "Programa Bolsa Família (PBF)" and id_ibge:
        ...
        try:
            ...
            if res.status_code == 200 and len(res.json()) > 0:
                ...
                data = [{
                    "TERRITÓRIO": local, "ANO": ano, "MÊS": mes,
                    "BPC_IDOSO_ATIVOS": int(qtd_beneficios * 0.4), # Estimativa visual
                    "BPC_PCD_ATIVOS": int(qtd_beneficios * 0.6),   # Estimativa visual
    ...
    # ---------------------------------------------------------
    # DADOS ESTRUTURAIS/MOCKADOS (CadÚnico e Censo SUAS)
    # ---------------------------------------------------------
    time.sleep(0.5)
